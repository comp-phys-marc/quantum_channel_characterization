...
@profile
def gen_one(i, json_dict, circuit):
    """
    Generates one sample.
    :param i: The number identifying the sample.
    :param json_dict: The samples data structure.
...
def generate_dataset(n_qubits, depth, n_samples):
    ...
    for i in range(n_samples):
        json_dict = gen_one(i, json_dict, circuit)
...
if __name__ == '__main__':
    ...
    for m in range(2, 3):
        ...
        for n in range(2, 5):
            print(f"Layers: {n}")
            f = open(f"data/two_thousand_training_dataset_{m}_qubits_{n}_layers.json", "w")

            json_dict = generate_dataset(m, n, 2000)
